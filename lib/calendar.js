...
async function createEvent({
  ...
  // Google Calendar API用のイベントオブジェクトを作成
  // undefinedフィールドや不正なフィールドを除去
  const event = {};

  if (summary !== undefined && summary !== null) {
    event.summary = summary;
  } else {
    throw new Error("イベントのタイトル(summary)が必要です");
  }
  ...
  if (start !== undefined && start !== null) {
    ...
    throw new Error("イベントの開始時刻(start)が必要です");
  }

  if (end !== undefined && end !== null) {
    event.end = { ...end };
  } else {
    throw new Error("イベントの終了時刻(end)が必要です");
  }
  ...
  console.log("ChronoClip: Event object for Google Calendar API:", event);
