...
function parseDate(text) {
  ...
  console.log("ChronoClip: Parsing date from text:", text);

  // 1. chrono-nodeによる解析を試行
  const chronoResult = parseWithChrono(text);
  if (chronoResult) {
    console.log("ChronoClip: Date parsed with chrono:", chronoResult);
    return chronoResult;
  }
  ...
  if (regexResult) {
    console.log("ChronoClip: Date parsed with regex:", regexResult);
    return regexResult;
  }

  console.log("ChronoClip: No date found in text");
  return null;
}
...
console.log("ChronoClip: Date parser module loaded");
