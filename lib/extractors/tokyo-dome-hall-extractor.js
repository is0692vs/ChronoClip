...
class ChronoClipTokyoDomeHallExtractor extends window.ChronoClipBaseExtractor {
  ...
  async extractAll(context) {
    try {
      ...
      if (events.length > 0) {
        ...
        result.description = firstEvent.description;
  ...
  extractEventsForSpecificTarget(doc, targetInfo) {
    ...
    if (eventDetails) {
      // 年月情報を取得
      const yearMonthInfo = this.extractYearMonth(doc);
   ...
   * @returns {Object}
   */
  extractYearMonth(document) {
    try {
      const yearMonthElement = document.querySelector(".c-ttl-set-calender");
      if (!yearMonthElement) {
  ...
  createEventObject(yearMonthInfo, dayInfo, eventDetail) {
    try {
      ...
      if (eventDetail.startTime) {
        const [hours, minutes] = eventDetail.startTime.split(":").map(Number);
        startDateTime = new Date(
          yearMonthInfo.year,
          yearMonthInfo.month - 1,
          dayInfo.day,
          hours,
          minutes
        );
      } else {
        // 時間が指定されていない場合は、その日の開始時刻を設定
        startDateTime = new Date(
          yearMonthInfo.year,
          yearMonthInfo.month - 1,
          dayInfo.day,
